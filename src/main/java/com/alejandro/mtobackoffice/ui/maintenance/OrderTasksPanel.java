 ...
 * terminada), generar las de un preventivo sobre un tramo (borrador o planificada) y modificar o
 * cancelar las abiertas. Iniciarlas y completarlas es trabajo de turno. Cada cambio avisa a la ficha,
 * que repinta el avance.
 */
class OrderTasksPanel extends LazyPanel {
    ...
    OrderTasksPanel(Supplier<OrderDto> order, MaintenanceClients clients, MaintenanceCatalogs catalogs, boolean canWrite, Runnable changed) {
        this.order = order;
    ...
    private Component rowActions(TaskDto task) {
        ...
        if (canWrite && task.isOpen()) {
            actions.add(MaintenanceUi.rowButton("task-edit-" + task.id(), VaadinIcon.EDIT, "Modificar",
                    click -> new TaskEditorDialog(order.get(), task, clients, catalogs, this::changed).open()));
